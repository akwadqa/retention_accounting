...
def execute(filters=None):
	...
	accounts = get_accounts_data(based_on, filters.get("company"), filters.get("project_type"))
	data = get_data(accounts, filters, based_on)
	columns = get_columns(filters)
	return columns, data


def get_accounts_data(based_on, company, project_type = None):
	if based_on == "Cost Center":
		...
		if project_type and project_type != "":
			return frappe.get_all("Project", fields=["name" , "project_name", "project_type"], filters={"company": company , "project_type" : ("like" , project_type)}, order_by="name")
		else:
			return frappe.get_all("Project", fields=["name" , "project_name", "project_type"], filters={"company": company}, order_by="name")
...
def get_columns(filters):
	...
	if filters.get("based_on") == "Project":
		...
		columns.insert(1, project_name)
		columns.insert(2, project_type)
